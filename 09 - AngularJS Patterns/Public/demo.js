(function () {
    var _ = { fn: function () { } };
    var mod = angular.module("externals", []);
    mod.constant("_", _);
}());
...
(function () {
    var mod = angular.module("moviesApp", ["externals"]);

    mod.factory("moviesSvc", function () {
        var movies = [
            {
                "id": 1,
    ...
    mod.controller("moviesCtrl", function ($scope, moviesSvc, $http, _) {
        $scope.movies = moviesSvc.query();

        $scope.delete = function (movie) {
            moviesSvc.delete(movie);
            _.fn();
        }
